...
ssh_info = {
    'ssh_address_or_host': (config.get(section='test_env', option='ssh_host'), 22),  # 指定ssh登录的跳转机的address
    'ssh_username': config.get(section='test_env', option='ssh_username'),  # 跳转机的用户
    'ssh_pkey': KEY_FILE,
    'remote_bind_address': (config.get(section='test_env', option='remote_bind_address'), 3306)
}

# DB
test_db_info = {
    'host': config.get(section='test_env', option='test_host'),
    'port': None,
    'user': config.get(section='test_env', option='test_user'),
    'passwd': config.get(section='test_env', option='test_passwd'),
    'db': config.get(section='test_env', option='test_db')
}

auto_db_info = {
    'host': config.get(section='test_env', option='auto_host'),
    'port': int(config.get(section='test_env', option='auto_port')),
    'user': config.get(section='test_env', option='auto_user'),
    'passwd': config.get(section='test_env', option='auto_passwd'),
    'db': config.get(section='test_env', option='auto_db')
}
